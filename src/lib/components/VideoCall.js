...
const VideoCall = ({ getTokenFromServer }) => {
  ...
  const toggleAudio = () => {
    localVideoTrack.disable({ audio: !audioState });
    setAudioState(!audioState);
};

const toggleVideo = () => {
    localVideoTrack.disable({ video: !video});
    setVideo(!video);
};
  ...
  return (
    ...
    <>
      ...
      {audioState && (
        <Icon name='microphone' size='big' onClick={toggleAudio} />
      )}
      {!audioState && (
        <Icon name='microphone slash' size='big' onClick={toggleAudio} />
      )}
      {video && <Icon name='video camera' size='big' onClick={toggleVideo} />}
      {!video && (
        <Icon name='video camera slash' size='big' onClick={toggleVideo} />
      )}
    </>
  );
};
